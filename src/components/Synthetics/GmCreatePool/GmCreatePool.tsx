...
import { GmConfirmationBox } from "../GmSwap/GmConfirmationBox/GmConfirmationBox";
import { getGmCreatePoolAvailableModes } from "./getGmCreatePoolAvailableModes";

import "./GmCreatePool.scss";
...
export function GmCreatePool(p: Props) {
  ...
  const [stage, setStage] = useState<"swap" | "confirmation" | "SynapChecked"  | "processing">();
  ...
  const availableModes = getGmCreatePoolAvailableModes(operation, marketInfo);
  ...
  useEffect(() => {
    ...
    Synaps.init({
      sessionId: '29a5df05-38e7-4a55-a3a1-0c3e90df74bb',
      onFinish: () => {
  ...
  return (
    <div className={`App-box GmCreatePool`}>
      ...
      >
        <div className={cx("GmCreatePool-form-layout", { reverse: isWithdrawal })}>
          <BuyInputSection
          ...
          >
            {secondTokenAddress ? (
              <TokenSelector
                ...
                onSelectToken={(token) => setSecondTokenAddress(token.address)}
                tokens={tokenOptions}
                infoTokens={infoTokens}
                className="GlpSwap-from-token"
                showSymbolImage={true}
        ...
        <ExchangeInfo className="GmCreatePool-info-section" dividerClassName="App-card-divider">
          <ExchangeInfo.Group>
            <ExchangeInfoRow
          ...
          <ExchangeInfo.Group>
            <div className="GmCreatePool-info-section">
              <GmFees
                isDeposit={isDeposit}
          ...
          {isHighPriceImpact && (
            <ExchangeInfo.Group>
              <Checkbox
                className="GmCreatePool-warning"
                asRow
